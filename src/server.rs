...
pub fn handle_client(mut stream: TcpStream, base_dir: &Path) -> io::Result<()> {
    ...
    loop {
        ...
        // Process data packets
        if let Some(ref path) = file_path {
            let seq_num = u32::from_be_bytes(buffer[..4].try_into().unwrap());
            let data = &buffer[4..amt];
            if seq_num == expected_seq {
                let mut file = File::options().create(true).append(true).open(path)?;
                file.write_all(data)?;
                stream.write_all(&seq_num.to_be_bytes())?;
                expected_seq = expected_seq.wrapping_add(1);
            } else {
                // Resend last ACK for out-of-order packets
                let ack = if expected_seq == 0 {
                    [0u8; 4] // Special case for the first packet
                } else {
                    (expected_seq - 1).to_be_bytes()
                };
...
pub fn main_tcp() -> io::Result<()> {
    let filter: &Filter = Filter::get_instance();
    let server_ip = filter.get_server_ip();
    let server_port = filter.get_server_port();
    let base_dir = Path::new(filter.get_base_dir());
