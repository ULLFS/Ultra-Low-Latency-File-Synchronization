...
use std::{env::temp_dir, error::Error, fs::{self, create_dir_all, File}, io::{BufRead, BufReader, BufWriter, Read, Seek, SeekFrom, Write}, path::Path};
...
fn ask_for_file(file: &String, _stream: &mut TcpStream){
    ...
    *cur_index += 1;
    ...
    if *cur_index >= 8{
        ...
        *cur_index = 0;
        ...
        // }
        match Path::new(&local_path).parent() {
            Some(x) => {
                if ! x.exists() {
                    create_dir_all(x);
                }
...
pub async fn processing(mut stream: &mut TcpStream, save_path : &str) {
                    ...
                    State::FileDeltaHash => {
                        ...
                        if state == State::FileDeltaData {
                            ...
                            if !fs::exists(save_path.to_string() + filepath.as_str()).expect("Failed to see if file exists") {
                                ...
                                ask_for_file(&filepath, &mut stream);
